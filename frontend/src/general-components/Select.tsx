...
export function Select({ options, value, onChange, placeholder = "Seleccionar", className = "" }: SelectProps) {
          ...
          {options.map((option) => (
            <button
              ...
              className={`w-full px-4 py-2 text-left font-medium transition-colors hover:bg-ui-secondary
                ${selected?.value === option.value ? "bg-ui-primary text-white" : "text-ui-primary"}`}
            >
              {option.label}
            </button>
          ))}
